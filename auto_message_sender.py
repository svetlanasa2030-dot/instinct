...
STARTUP_GAP = 60  # 1 minute between initial messages
...
class AutoMessageApp:
    ...
    def build_ui(self):
        ...
        ttk.Label(main,text="При запуске сообщения отправляются по очереди с интервалом 1 минута. После первой отправки каждое сообщение повторяется по своему заданному интервалу.",wraplength=780).pack(anchor="w",pady=(5,12))
        ...
        self.global_timer=ttk.Label(main,text="Таймер: —",font=("Segoe UI",10,"bold")); self.global_timer.pack(anchor="w")
    ...
    def get_config(self):
        ...
        for _,mv,iv,_ in self.rows:
            ...
            try:
                ...
                raise ValueError(f"Некорректный интервал для: {msg}")
    ...
    def start(self):
        ...
        self.status.config(text="Запущено: начальная отправка по очереди, интервал 1 минута")
        self.next_send=[None]*len(config)
    ...
    def update_timers(self):
        ...
        if left:
            self.global_timer.config(text=f"Таймер до ближайшей отправки: {self.fmt(min(left))}")
        else:
            self.global_timer.config(text="Идёт начальная отправка по очереди...")

        self.root.after(500,self.update_timers)

    def worker(self,config):
        # Начальная очередь: сообщение 1 сразу, сообщение 2 через 1 минуту,
        # сообщение 3 ещё через 1 минуту и т.д.
        for i,(message,interval) in enumerate(config):
            ...
            # После первоначальной отправки запускается индивидуальный таймер.
            self.next_send[i]=time.monotonic()+interval

        self.root.after(0,self.update_timers)

        # Далее каждое сообщение повторяется через свой заданный интервал.
        while self.running:
            now=time.monotonic()
            for i,(message,interval) in enumerate(config):
