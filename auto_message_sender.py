...
class AutoMessageApp:
    ...
    def build_ui(self):
        ...
        ttk.Label(main, text="Клик → Ctrl+V → Enter. Сообщения берутся из полей программы и отправляются по своим интервалам.", wraplength=780).pack(anchor="w", pady=(5,12))
        ...
        ttk.Label(h,text="Сообщение",width=44).pack(side="left")
        ttk.Label(h,text="Интервал (мин.)",width=15).pack(side="left")
        ttk.Label(h,text="Таймер",width=15).pack(side="left")
    ...
    def remove_row(self,row):
        ...
        for item in self.rows[:]:
            if item[0]==row:
                row.destroy(); self.rows.remove(item); break
    ...
    def select_target(self):
        ...
        def choose(e):
            ...
            self.status.config(text="Точка выбрана"); overlay.destroy()
    ...
    def get_config(self):
        ...
        for _,mv,iv,_ in self.rows:
            ...
            if not msg: continue
            try:
                minutes=float(iv.get().replace(",","."))
                if minutes<=0: raise ValueError
            except ValueError: raise ValueError(f"Некорректный интервал для: {msg}")
    ...
    def start(self):
        ...
        self.next_send=[time.monotonic()+interval for _,interval in config]
    ...
    def send_message(self,message):
        ...
        pyautogui.click(x,y)
        time.sleep(0.15)
        # Сначала копируем текст сообщения из программы в буфер обмена.
        pyperclip.copy(message)
        # Затем вставляем именно Ctrl+V и отправляем Enter.
